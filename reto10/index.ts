...
de forma cíclica hasta llegar a la altura indicada. Como mínimo siempre nos pasarán uno.
...
function createChristmasTree(ornaments: string, height: number): string {
    let christmasTree = "";
    let ornamentsArr = ornaments.repeat(height * height).split("");

    for (let index = 1; index <= height; index++) {
        christmasTree += " ".repeat(height - index);
        christmasTree += ornamentsArr.slice(0, index).join(" ");
        christmasTree += "\n";
        ornamentsArr = ornamentsArr.slice(index);
    }
...
console.log(createChristmasTree("x", 20));
